...
public class DashbordController {
  ...
  @GetMapping("/getTabCreerConsulter")
    public List<CreerConsulter> getTableData() {

        //return gestionDevCreerConsulter.getGestionDevCreerConsulter();
      List<CreerConsulter> list = new ArrayList<CreerConsulter>();
      CreerConsulter tt = new CreerConsulter();
      tt.setECU("amal");
      tt.setCDC("masgh");
      tt.setDLL("dll");
      tt.setN("n");
...
public ResponseEntity<List<String>> getAll() {
    List<DEV> devs = devRepository.findByName("nom_dev");
    List<String> devNames = devs.stream().map(DEV::getNomDev).collect(Collectors.toList());
    return ResponseEntity.ok().body(devNames);
}*/
  @GetMapping("/getdev")
...
// Print the query with parameter values
    ...
    public ResponseEntity<?> AjoutVehicule(@RequestBody VehidDto vehidDto){
       Vehid vehid=new Vehid();
       vehid.setNomVeh(vehidDto.getNomVeh());
       vehid.setNomInterne(vehidDto.getNomInterne());
       vehid.setMarque(vehidDto.getMarque());
       vehid.setTestGlobal(vehidDto.getTestGlobal());
       vehid.setMsgDiag(vehidDto.getMsgDiag());
       vehid.setGrpMarq(vehidDto.getGrpMarq());
       vehid.setFro(vehidDto.getFro());
       vehid.setT(vehidDto.getT());
       vehid.setOnlyElec(vehidDto.getOnlyElec());
       vehid.setAvertissElec(vehidDto.getAvertissElec());
        vehidRepo.save(vehid);
        return ResponseEntity.ok().body(new AuthResponse(true,"vehicule cree"));
    }
@GetMapping("/getallVoiture")
    public List<Vehid> getAllvoiture(){
      return vehidRepo.findAll();
}
/*@PutMapping("/update-vehicule")
    public ResponseEntity<?> updateVehicule(@RequestBody VehidDto vehidDto) {
        Optional<Vehid> vehiculeOptional = vehidRepo.findById(vehidDto.getCodeVeh());
        if (vehiculeOptional.isPresent()) {
            Vehid vehid = vehiculeOptional.get();
            vehid.setNomVeh(vehidDto.getNomVeh());
            vehid.setNomInterne(vehidDto.getNomInterne());
            vehid.setCodeVeh(vehidDto.getCodeVeh());
            vehid.setTestGlobal(vehidDto.getTestGlobal());
            vehid.setMsgDiag(vehidDto.getMsgDiag());
            vehid.setGrpMarq(vehidDto.getGrpMarq());
            vehid.setFro(vehidDto.getFro());
            ...
            vehid.setAvertissElec(vehidDto.getAvertissElec());
            vehidRepo.save(vehid);
            return ResponseEntity.ok().body(new AuthResponse(true,"vehicule update"));
        } else {
    ...
    public ResponseEntity<?> updateVehicule(@RequestBody Vehid vehicule) {
        Vehid updatedVehicule = vehidRepo.save(vehicule);
